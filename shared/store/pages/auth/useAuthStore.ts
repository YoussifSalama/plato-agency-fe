...
const useAuthStore = create<IAuthStore>((set) => {
    ...
    return {
        ...
        signup: async (f_name, l_name, email, user_name, password) => {
            set({ loadingSignup: true });
            try {
                const result = await apiClient.post("/agency/signup", {
                    f_name,
                    l_name,
                    email,
                    user_name,
                ...
                errorToast(resolveErrorMessage(error, "Failed to create account."));
                return false;
            } finally {
        ...
        verifyPasswordResetOtp: async (email, otp) => {
            ...
            try {
                ...
                const isValid = Boolean(
                    result.data?.valid ?? result.data?.data?.valid
                );
                if (!isValid) {
                    errorToast(
                        resolveResponseMessage(
        ...
        verifyAccountToken: async (token) => {
            try {
                ...
                const isValid = Boolean(
                    result.data?.data?.valid ?? result.data?.valid
                );
                if (isValid) {
                    successToast(
                        resolveResponseMessage(
        ...
        resendVerificationToken: async (token) => {
            try {
                const result = await apiClient.post("/agency/resend-verification", { token });
                successToast(
                    resolveResponseMessage(
                        result,
                ...
                errorToast(resolveErrorMessage(error, "Failed to resend verification email."));
                return false;
            }
        },
        logout: async () => {
            try {
                ...
                if (refreshToken) {
                    await apiClient.post("/agency/logout", { refresh_token: refreshToken });
                }
            } catch {
